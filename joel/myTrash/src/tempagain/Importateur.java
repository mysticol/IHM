...
public class Importateur {
    ...
    static public void main(String[] args) throws Exception {
        ...
        applicationConfig.addActionAlias("import",
    ...
    static public void importToWikitty() throws Exception {
        ...
        // String wikittyService =
        // applicationConfig.getOption(WITTY_SERVICE_KEY);
        ...
        for (File fileToTransform : toTransfert) {
            listWikitty.add(fileToWikitty(fileToTransform, dir));
        }
     ...
     * 
     * boolean noRecur = applicationConfig
     * .getOptionAsBoolean(WikittyPublication.NO_RECURSION_KEY); File dir =
     * applicationConfig.getOptionAsFile(WikittyPublication.DIRECTORY_KEY);
     * String wikittyService =
     * applicationConfig.getOption(WikittyPublication.WITTY_SERVICE_KEY);
     * boolean hessianProtocole = applicationConfig
     * .getOptionAsBoolean(WikittyPublication.HESSIAN_PROTOCOL_KEY);
     * 
     * log.info("checkout : wikittyservice: " + wikittyService + " noresursion="
     * + noRecur + " directory= " + dir.getAbsolutePath() + "Label a checkout "
     * + label + "HessianProtocol=" + hessianProtocole);
     * 
     * if (hessianProtocole) {
     * applicationConfig.setOption("wikitty.WikittyService.components",
     * "org.nuiton.wikitty.services.WikittyServiceHessianClient"); } else {
     * applicationConfig.setOption("wikitty.WikittyService.components",
     * "org.nuiton.wikitty.services.WikittyServiceCajoClient"); }
     * 
     * WikittyProxy remoteWikittyService = new WikittyProxy(
     * WikittyServiceFactory.buildWikittyService(applicationConfig));
     * WikittyPublicationFileSystem localWikittyService = new
     * WikittyPublicationFileSystem( dir, !noRecur, label);
     * 
     * // Construct the criteria Criteria labelCriteria; Search mainRequest =
     * Search.query(); Search subRoqu = mainRequest.or();
     * 
     * // must have the type of wikittypubtext/wikittypubdata
     * subRoqu.exteq(WikittyPubText.EXT_WIKITTYPUBTEXT).exteq(
     * WikittyPubData.EXT_WIKITTYPUBDATA); if (noRecur) { // and extension with
     * the name strictly equals to the label (no // recursivity) labelCriteria =
     * mainRequest.exteq(WikittyLabel.EXT_WIKITTYLABEL)
     * .eq(WikittyLabel.FQ_FIELD_WIKITTYLABEL_LABELS, label) .criteria();
     * 
     * } else { // and extension with the name that containt the label
     * (recursivity) labelCriteria =
     * mainRequest.exteq(WikittyLabel.EXT_WIKITTYLABEL)
     * .sw(WikittyLabel.FQ_FIELD_WIKITTYLABEL_LABELS, label) .criteria(); }
     * 
     * // request to the proxy PagedResult<Wikitty> pageResult =
     * remoteWikittyService .findAllByCriteria(labelCriteria);
     * 
     ...
     * 
     * // write the wikities localWikittyService.store("", wikittiesToWrite,
     * true);
     * 
    ...
    /*
     * on va commencer par vérifier les arguments ''wp checkout [--norecursion]
     * [url du WikittyService] [Label à extraire] [directory local d'accueil]''
     * on doit avoir trois string dans le unparsed: url, label, directory et
    ...
    static public Wikitty fileToWikitty(File fileToTransform, File starts)
        ...
        /*
         *  mfortun-2011-04-08 find a better way to do this ?
         */
        ...
        String label = path.replaceAll(File.separator,
                WikittyPublicationFileSystem.WIKITTYLABEL_SEPARATOR);
    ...
    static public String mimeTypeForExtension(String ext) {
        /*
         *  mfortun-2011-04-08 really implements this method create a double
    ...
    static public String extensionFormimeType(String mime) {
        /*
         *  mfortun-2011-04-11 really implements this method
         */
    ...
    static public boolean isMimeWikittyPubText(String mime) {
        /*
         *  mfortun-2011-04-08 really implements this method
         */
